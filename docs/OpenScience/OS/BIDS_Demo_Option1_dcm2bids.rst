...
For this tutorial we will be using the dcm2bids BIDS converter; however, as you become familiarized with BIDS you may find a different converter that suits your fancy.
...
The starred acquistions show the functiBIDS Tutorial #1: dcm2bids
*************
...
Clean project folderBIDS Tutorial #1: dcm2bids
*************
...
::
  ...
  anat-T1w (0)
  fmap-SE-AP (1)
  fmap-SE-PA (2)BIDS Tutorial #1: dcm2bids
*************
...
Now that you have arranged your data in BIDS format, you are ready to begin using BIDS apps, such as MRIQC and fMRIPrep. To see how to use MRIQC, click the ``Next`` button.
onal data corresponding to the field maps. Functional indices 4 & 6 correspond to the first field map pair, and functional indices 11 & 13 correspond to the second field map pair. Note that single-band reference (sbref) acquisitions are not included in the ``IntendedFor`` field. If a single pair of field maps (regardless of type) were collected and the participant did not leave and re-enter the scanner during the session, then the ``IntendedFor`` list would contain the indices of all the functional acquisitions.
